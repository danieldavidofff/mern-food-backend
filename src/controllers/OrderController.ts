...
const createLineItems = (checkoutSessionRequest: CheckoutSessionRequest, menuItems: MenuItemType[]) => {
  ...
  //Trying to find the menuItem for a given cartItem based on the ids
  const lineItems = checkoutSessionRequest.cartItems.map((cartItem) => {
    ...
    //Tip line_item that is how the stripe object is defiend
...
const createSession = async (lineItems: Stripe.Checkout.SessionCreateParams.LineItem[], orderId: string, deliveryPrice: number, restaurantId: string) => {
  const sessionData = await STRIPE.checkout.sessions.create({
    ...
    success_url: `${FRONTEND_URL}/oder-status?success=true`,
